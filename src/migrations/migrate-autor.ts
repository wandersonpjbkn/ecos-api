...
const run = async () => {
  ...
  console.log('👤 Migrando campo autor...')

  for (const book of books) {
    const raw = book.autor
    ...
    if (typeof raw !== 'string' || !raw.trim()) {
      console.warn(`⚠️  "${book.titulo}" sem autor válido — ignorado`)
      skipped++
      continue
    }
  ...
  console.log('\n─────────────────────────────────────')
  console.log('📊 Resumo:')
  console.log(`   Autor → ${converted} convertidos, ${skipped} ignorados, ${errors} erros`)
  console.log(
